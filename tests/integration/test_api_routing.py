...
class TestRoutingAPI:
    ...
    def test_route_task_with_rules_engine(
        ...
        decision = db_session.query(RoutingDecision).filter(
            RoutingDecision.task_id == task.id
        ).first()
    ...
    def test_route_task_auto_assigns_destination(
        self, api_client: TestClient, db_session: Session
    ):
        """Test that routing automatically assigns task to destination."""
        task = TaskFactory.create(
            session=db_session,
            title="Task about routing and queues",
            project=None
        )
    ...
    def test_get_routing_suggestions(
        self, api_client: TestClient, db_session: Session
    ):
    ...
    def test_route_task_with_llm_fallback(
        self, api_client: TestClient, db_session: Session
    ):
        ...
        task = TaskFactory.create(
            session=db_session,
            title="Very obscure unusual task",
            project=None,
            tags={}
        )
        ...
        response = api_client.post(
            f"/api/v1/tasks/{task.id}/route",
            json={"strategy": "rules", "fallback": "llm"}
        )
        assert_response_success(response)
    ...
    def test_route_nonexistent_task(self, api_client: TestClient):
        ...
        response = api_client.post(
            "/api/v1/tasks/nonexistent/route",
            json={"strategy": "rules"}
        )
        assert_response_error(response, 404)

    def test_route_already_routed_task(
        self, api_client: TestClient, db_session: Session
    ):
...
class TestRoutingDecisions:
    ...
    def test_get_routing_decision_by_task(
        self, api_client: TestClient, db_session: Session
    ):
    ...
    def test_list_routing_decisions(
        self, api_client: TestClient, db_session: Session
    ):
...
class TestRoutingFeedback:
    ...
    def test_provide_positive_feedback(
        self, api_client: TestClient, db_session: Session
    ):
        ...
        response = api_client.post(
            f"/api/v1/tasks/{task.id}/routing-feedback",
            json=feedback_data
        )
        ...
        feedback = db_session.query(TaskFeedback).filter(
    ...
    def test_feedback_affects_future_routing(
        self, api_client: TestClient, db_session: Session
    ):
        ...
        task1 = TaskFactory.create(
            session=db_session,
            title="Task about orchestration",
            project="hopper"
        )
        ...
        task2 = TaskFactory.create(
            session=db_session,
            title="Another orchestration task",
            project=None
        )

        response = api_client.post(
            f"/api/v1/tasks/{task2.id}/route",
            json={"strategy": "rules", "use_feedback": True}
        )
        assert_response_success(response)
    ...
    def test_get_routing_feedback(
        self, api_client: TestClient, db_session: Session
    ):
        """Test retrieving routing feedback for a task."""
        task = TaskFactory.create(session=db_session)
        feedback = TaskFeedbackFactory.create(
            session=db_session,
            task_id=task.id
        )
...
class TestRoutingAnalytics:
    ...
    def test_get_routing_accuracy_metrics(
        self, api_client: TestClient, db_session: Session
    ):
        ...
        for i in range(8):
            ...
            TaskFeedbackFactory.create_positive(
                session=db_session,
                task_id=task.id
            )
    ...
    def test_get_destination_popularity(
        self, api_client: TestClient, db_session: Session
    ):
    ...
    def test_get_average_confidence_by_strategy(
        self, api_client: TestClient, db_session: Session
    ):
...
class TestRuleEvaluation:
    ...
    def test_evaluate_rules_for_task(
        self, api_client: TestClient, db_session: Session
    ):
    ...
    def test_test_rule_against_task(
        self, api_client: TestClient, db_session: Session
    ):
