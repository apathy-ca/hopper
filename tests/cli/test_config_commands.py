...
def test_init_command(runner: CliRunner, temp_config_dir: Path, monkeypatch) -> None:
    ...
    result = runner.invoke(
        cli, ["init", "--endpoint", "http://localhost:8000", "--non-interactive"]
    )
...
def test_config_set(runner: CliRunner, mock_config, temp_config_dir: Path) -> None:
    ...
    result = runner.invoke(
        cli, ["config", "set", "api.endpoint", "http://newhost:9000"], obj=mock_config
    )
...
def test_auth_login_with_token(
    runner: CliRunner, mock_config, temp_config_dir: Path, mock_client: Mock
) -> None:
